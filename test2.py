...
# Fungsi untuk melakukan backoff eksponensial dengan jeda acak
def exponential_backoff(retries, base_delay=2, max_delay=30):
    delay = min(base_delay * (2 ** retries), max_delay)
    time.sleep(delay + random.uniform(0, 1))  # Delay acak untuk menghindari pola permintaan tetap
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if response.status_code == 429:
                # Tangani status 429 (Too Many Requests)
                logger.warning(f"Terkena pembatasan (HTTP 429) untuk '{query}', mencoba lagi...")
                exponential_backoff(attempt)  # Menunggu dengan backoff eksponensial
                continue  # Lanjutkan percakapan ulang
...
def search_keywords_parallel(domain, keywords):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:  # Batasi jumlah thread aktif
...
def save_results_to_html(results, failed_results):
    ...
    if results:
        for result in results:
            ...
            <tr>
                ...
                <td><a href="{result['link']}" target="_blank">Kunjungi</a></td>
            </tr>
            """
    ...
    logger.info(f"Hasil pencarian telah disimpan di {file_path}")

# Domain dan kata kunci yang akan digunakan
domain = "kalselprov.go.id"
keywords = [
...
# Menyimpan hasil pencarian ke file HTML
